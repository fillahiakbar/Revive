    ...
    <div class="min-h-screen flex items-center justify-center px-4 py-6 sm:px-6">
        <div class="w-full max-w-md sm:max-w-lg md:max-w-xl px-4 sm:px-6 py-8 sm:py-10 rounded-xl">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-4 sm:space-y-6">
                ...
                {{-- Remember Me + Forgot --}}
                <div class="flex flex-col sm:flex-row justify-between items-center text-xs sm:text-sm text-white space-y-2 sm:space-y-0">
                    <label class="flex items-center space-x-2 space-x-reverse">
                        <input type="checkbox" name="remember" class="form-checkbox text-red-600 rounded h-4 w-4">
                        <span>تذكّرني</span>
                    </label>
                    @if (Route::has('password.request'))
                        <a class="text-blue-400 hover:text-blue-300 hover:underline" href="{{ route('password.request') }}">
                            هل نسيت كلمة المرور؟
                        </a>
                    @endif
                </div>
